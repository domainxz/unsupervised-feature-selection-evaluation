...
for i = 1:m
    ...
    A = rand(M, size(benchmark2, 2));
    for alpha=rp
        for beta=rp
            [~,fsm] = method(ZX, L, F, A, 20, 10^alpha, 10^beta, 10^8);
            for num=feature_num
...
for i = 1:a
    ...
    for x = 1:b
        for y = 1:b
            for fnum = 1:6 
                load(sprintf('features/%s-%s(%d_%d)-%d.mat', ds{i,1}, func2str(method), rp(x), rp(y), feature_num(fnum)));
                for iter = 1:iterval
